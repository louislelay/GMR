...
def process_file(
    ...
    while check_memory():
        ...
        num_pause += 1
        if num_pause > 10:
            print("[ERROR] Memory usage is still high after 10 pauses. Exiting.")
            return
    ...
    if HEIGHT_ADJUST:
        ...
        root_pos[:, 2] = (
            root_pos[:, 2] - lowerst_height + ground_offset
        )  # make sure motion on the ground
...
def main():
    ...
    for dirpath, _, filenames in os.walk(src_folder):
        for filename in natsorted(filenames):
            ...
            if filename.endswith((".pkl", ".npz")):
                ...
                tgt_file_path = smplx_file_path.replace(src_folder, tgt_folder).replace(
                    ".npz", ".pkl"
                )
    ...
    with mp.Pool(args.num_cpus) as pool:
        pool.starmap(
            process_file, [args + (total_files, verbose) for args in args_list]
        )
