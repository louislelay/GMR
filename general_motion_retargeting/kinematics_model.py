...
class KinematicsModel:
    ...
    def _parse_xml(self):
        ...
        assert self._rot_unit in ["degree", "radian"], (
            f"Invalid rotation unit: {self._rot_unit}"
        )
        ...
        def _add_xml_body(xml_node, parent_index, body_index):
            ...
            if body_index == 0:
                ...
                if num_joints == 0:
                    ...
                    _axis = np.fromstring(
                        curr_joints[0].attrib.get("axis"), dtype=float, sep=" "
                    )
    ...
    def rot_to_dof(self, rot):
        ...
        for j in range(1, self.num_joint):
            ...
            if joint.dof_dim == 0:
                continue
            joint_rot = rot[..., j - 1, :]
            dof[..., joint.dof_idx : joint.dof_idx + joint.dof_dim] = joint.rot_to_dof(
                joint_rot
            )
