...
try:
    ...
    print(
        "[bold red]xrobotoolkit_sdk not found, skip for now. If you do not use XRobotStreamer, it's fine.[/bold red]"
    )
import json
import os
...
class XRobotStreamer:
    ...
    def get_left_hand_data(self):
        ...
        for i, joint_name in enumerate(self.hand_joint_names):
            pos = [
                left_hand_tracking_state[i][0],
                left_hand_tracking_state[i][1],
                left_hand_tracking_state[i][2],
            ]  # xyz
            rot = [
                left_hand_tracking_state[i][6],
                left_hand_tracking_state[i][3],
                left_hand_tracking_state[i][4],
                left_hand_tracking_state[i][5],
            ]  # scalar first wxyz
    ...
    def get_right_hand_data(self):
        ...
        for i, joint_name in enumerate(self.hand_joint_names):
            pos = [
                right_hand_tracking_state[i][0],
                right_hand_tracking_state[i][1],
                right_hand_tracking_state[i][2],
            ]  # xyz
            rot = [
                right_hand_tracking_state[i][6],
                right_hand_tracking_state[i][3],
                right_hand_tracking_state[i][4],
                right_hand_tracking_state[i][5],
            ]  # scalar first wxyz
    ...
    def get_raw_body_data(self):
        ...
        if xrt.is_body_data_available():
            ...
            body_accelerations = (
                xrt.get_body_joints_acceleration()
            )  # ax, ay, az, wax, way, waz
    ...
    def coordinate_transform_unity_data(self, body_pose_dict):
        ...
        for body_name, value in body_pose_dict.items():
            ...
            orientation = quat_mul_np(
                rotation_quat, np.array([qw, qx, qy, qz]), scalar_first=True
            )
...
class XRobotRecorder:
    ...
    def _load_and_process_data(self):
        ...
        print(
            f"Loaded {len(self.video_frames)} video frames and {len(self.tracking_data)} tracking frames"
        )
        ...
        for line_idx, line in enumerate(lines[1:], 1):
            ...
            if line:
                try:
                    ...
                    print(
                        f"Warning: Could not parse tracking data on line {line_idx + 1}: {e}"
                    )
    ...
    def _preprocess_all_data(self):
        ...
        for video_frame_idx in range(total_video_frames):
            ...
            video_frame_timestamp = self.initial_timestamp + (
                video_frame_idx * frame_duration_ns
            )
            ...
            left_hand_data = self._process_left_hand_data(
                nearest_mocap_frame, video_frame_idx
            )
            right_hand_data = self._process_right_hand_data(
                nearest_mocap_frame, video_frame_idx
            )
                    ...
                    if i < len(joints):
                        ...
                        if "p" in joint_data:
                            ...
                            try:
                                ...
                                print(
                                    f"Warning: Could not parse joint {i} ({joint_name}): {e}"
                                )
                                continue
                ...
                if i < len(joint_locations):
                    ...
                    if "p" in joint_data:
                        ...
                        try:
                            ...
                            print(
                                f"Warning: Could not parse left hand joint {i} ({joint_name}): {e}"
                            )
                            continue
    ...
    def _process_right_hand_data(self, frame_data, frame_idx):
        ...
        if "rightHand" in hand_data and "HandJointLocations" in hand_data["rightHand"]:
            ...
            # If hand is not active, use previous frame's data
            if not is_active:
                if frame_idx > 0 and frame_idx - 1 < len(
                    self.processed_right_hand_data
                ):
                    return self.processed_right_hand_data[frame_idx - 1].copy()
                # If no previous frame, continue with empty data
                ...
                if i < len(joint_locations):
                    ...
                    if "p" in joint_data:
                        ...
                        try:
                            ...
                            print(
                                f"Warning: Could not parse right hand joint {i} ({joint_name}): {e}"
                            )
                            continue
    ...
    def _process_controller_data(self, frame_data):
        ...
        if "leftController" in controller_data:
            ...
            if "inputState" in left_ctrl:
                ...
                result["LeftController"]["index_trig"] = input_state.get(
                    "indexTrigger", 0.0
                )
                result["LeftController"]["grip"] = input_state.get("handTrigger", 0.0)
                result["LeftController"]["key_one"] = input_state.get(
                    "menuButton", False
                )
                thumbstick = input_state.get("thumbstick", {})
                result["LeftController"]["axis"] = [
                    thumbstick.get("x", 0.0),
                    thumbstick.get("y", 0.0),
                ]
                result["LeftController"]["axis_click"] = input_state.get(
        ...
        if "rightController" in controller_data:
            ...
            if "inputState" in right_ctrl:
                ...
                result["RightController"]["index_trig"] = input_state.get(
                    "indexTrigger", 0.0
                )
                result["RightController"]["grip"] = input_state.get("handTrigger", 0.0)
                result["RightController"]["key_one"] = input_state.get(
                    "menuButton", False
                )
                thumbstick = input_state.get("thumbstick", {})
                result["RightController"]["axis"] = [
                    thumbstick.get("x", 0.0),
                    thumbstick.get("y", 0.0),
                ]
                result["RightController"]["axis_click"] = input_state.get(
                    "thumbstickClick", False
                )
    ...
    def set_frame_index(self, index):
        ...
        if 0 <= index < self.get_total_frames():
            ...
            raise IndexError(
                f"Frame index {index} out of range [0, {self.get_total_frames() - 1}]"
            )
    ...
    def get_human_height(self):
        ...
        print(
            f"Estimated human height: {estimated_height:.2f}m (from {valid_frames} valid frames)"
        )
