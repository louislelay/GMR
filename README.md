...
2. On your own PC, 
    ...
    - Build PICO PC Service SDK and Python SDK for PICO streaming (installs
      into GMR's `.venv` by default; activate a conda env first to install
      there instead):
        ```bash
...
> [!NOTE]
> NOTE: both `.npz` and `.pkl` SMPL-X body models are supported; GMR detects which one is present automatically.
