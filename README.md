...
GMR does not install platform-specific device SDKs. Install
`xrobotoolkit_sdk` for PICO or the Xsens `xsens_mvn_robot` adapter for Xsens;
each binding is imported only when its source is opened.
...
Profiles use the named v2 format documented in
...
GMR is licensed under the [MIT License](LICENSE).
